...
def fetch_and_store_prices():
    ...
    # Esta búsqueda de respaldo (¿cuál fue el último precio válido alguna vez
    # visto?) hace UNA consulta a Supabase por cada item pendiente. Con un
    # catálogo grande, si muchos items quedan sin precio nuevo a la vez, esto
    # puede disparar cientos de consultas individuales en una sola corrida del
    # cron — eso fue lo que causó el worker OOM/timeout. Se acota un tope por
    # corrida: los items que no alcancen esta vez quedan con su último valor
    # de precios_actuales (que igual no se pisa con None) y se procesan en la
    # próxima corrida — no se pierde el dato, solo se reparte el trabajo.
    MAX_FALLBACK_LOOKUPS_POR_CORRIDA = 40
    pendientes_sell_items = set(list(pendientes_sell_items)[:MAX_FALLBACK_LOOKUPS_POR_CORRIDA])
    pendientes_buy_items = set(list(pendientes_buy_items)[:MAX_FALLBACK_LOOKUPS_POR_CORRIDA])
