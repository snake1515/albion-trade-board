...
def api_compras():
    ...
    # cantidad_vendida / cantidad_restante / ganancia_acumulada.
    for c in compras:
        ventas = ventas_por_compra.get(c["id"], [])
        cantidad_vendida = sum(float(v["cantidad"]) for v in ventas)
        c["ventas"] = ventas
        c["cantidad_vendida"] = cantidad_vendida
        c["cantidad_restante"] = round(float(c["cantidad"]) - cantidad_vendida, 6)
        c["ganancia_acumulada"] = round(sum(float(v["ganancia"]) for v in ventas), 2)
        c["vendido"] = c["cantidad_restante"] <= 0.0001
...
def api_compras_vender(compra_id):
    ...
    ya_vendida = sum(float(v["cantidad"]) for v in ventas_existentes)
    restante = round(cantidad_total - ya_vendida, 6)

    # Si no mandan cantidad, se asume que están vendiendo todo lo que queda
    # (mantiene compatible el flujo de "venta completa" de antes).
    cantidad = float(body.get("cantidad") or restante)
    if cantidad <= 0:
        return jsonify({"error": "no queda cantidad pendiente por vender en esta compra"}), 400
    if cantidad - restante > 0.0001:
        return jsonify({"error": f"solo quedan {restante} unidades pendientes, no se pueden vender {cantidad}"}), 400

    # precio_venta que envía el usuario es el precio bruto (al que vendió/listó);
    # se descuenta el impuesto de mercado antes de calcular la ganancia, igual
    # que se hace para las compras pendientes en estadoRentabilidad().
    precio_venta_neto = float(precio_venta) * (1 - float(tax_pct) / 100)
    ganancia = round((precio_venta_neto - precio_oro) * cantidad, 2)
    ...
    restante_despues = round(restante - cantidad, 6)
    if restante_despues <= 0.0001:
        # Se agotó la cantidad comprada — se marca la compra como cerrada.
        # (precio_venta/ganancia legacy quedan con los de la última venta,
        # ya no son la fuente de verdad — eso ahora vive en compras_ventas.)
...
def api_compras_venta_delete(venta_id):
    ...
    if not venta.data:
        return jsonify({"error": "no encontrada"}), 404
    compra_id = venta.data["compra_id"]
    supabase.table("compras_ventas").delete().eq("id", venta_id).execute()
    # Si la compra ya estaba marcada como vendida y se borra una de sus ventas,
    # vuelve a quedar pendiente (con lo que sobre de cantidad_restante).
    supabase.table("compras_manual").update({
        "vendido": False,
        "fecha_vendida": None,
    }).eq("id", compra_id).execute()
